  ...
  fn = {
      ...
      hasDataAttr('[data-tab-label]',function($ctx){
        ...
        $('body').on('click','[data-tab-label]',function() {
          ...
          $this
            ...
            // 찾기 - 현재 탭에서 찾기가 체크되어 있으면..
            var $searchNowTab = $("#searchNowTab");
            
	  		if(typeof $searchNowTab != 'undefined') {
				if($searchNowTab.prop("checked")) {
					// 현재 탭 아이디 저장
					$searchNowTab.val(dataObj.target);
					
					// 현재 탭 표시
					$("#findResultTab").val($this.find('>a>em').text());
    ...
    toggleLinkList : function() {
      hasDataAttr('[data-toggle-link-list]',function($ctx){
        $ctx.each(function() {
          var $this = $(this);
          var dataObj = $this.data('toggle-link-list');
          var $toggleTarget = $('[data-link-list="' + dataObj + '"]');

          $this.on('click',function() {
            if ($this.hasClass('expanded')) {
            	var nlinkHeight = $toggleTarget.find('a').height();
          ...
          $this.on('click',function() {
            if (dataObj.way == 'radio-text') {
              if (dataObj.value === true && $this.prop('checked')) {
                ...
                if($target.val() == ''){
                	if(typeof dataObj.word == 'undefined'){//init.content
                		$target.val('해당 콘텐츠가 이해하기 쉽게 작성되었습니까?');	
                	}else{//qna
                		$target.val('문의하신 내용에 적합한 답변이 되었습니까?');	
                	}
                }
    ...
    form : function () {
      ...
      hasDataAttr('[data-select="true"]',function($ctx){
        $ctx.each(function() {
          ...
          if ( nvl($(this).attr("data-extend"), "") == "false" ) {	// 2023.12 data-extend가 false인 경우에는 "extend-custom-select" css가 적용되지 않게 customClass 공백 처리 / P051230 / 고도화 프로젝트			
          	$(this).selectOrDie({customClass: ''});
		  }else{
          	$(this).selectOrDie();
          }	
        });
          ...
          .click(function(e){
            ...
            if (hasClz == 'depth1') {
              $this.toggleClass('on');
				...
				if($this.children('#adm_dpt2').length > 0)
				{
	                
					$this.toggleClass('on');
                		
                	$lnb.find('.depth2 li').not("#adm_dpt1").removeClass('on');
				}
				else{
	              $lnb.find('.depth2 li').removeClass('on');
	              $this.addClass('on');
                }
            } else if (hasClz == 'depth3') {
              $lnb.find('.depth3 li').removeClass('on');
              $this.addClass('on');
            }
